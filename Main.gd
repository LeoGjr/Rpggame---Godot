...
var novoinimigo = preload("res://Inimigo.tscn")
var totalinimigos = 0

var novotesouro = preload("res://tesouro.tscn")
...
func _on_criainimigo_timeout():
	if totalinimigos < 20:
		...
		add_child(inimigo)
		inimigo.position.x = rand_range(1000,4000)
		inimigo.position.y = rand_range(500,3500)
		totalinimigos += 1
...
func _on_criatesouro_timeout():
	if tesourosnatela <= 7:
		...
		add_child(tesouro)
		tesouro.position.x = rand_range(1000,4000)
